...
REPORT_PDF = "release/Agentic_Finance_Report_v1.0-RC6.pdf"   # swap for the final release at launch
...
if PRELAUNCH:
    ...
    full_md = (repo / "Agentic_Finance_Report.full.md").read_text(encoding="utf-8")
    (root / "agentic-finance-report.md").write_text(full_md, encoding="utf-8")
    shutil.copyfile(repo / "Agentic_Finance_Report.summary.md", root / "agentic-finance.summary.md")
...
Co-authors: TensorX, AMINA Bank, Solana Foundation, APEX:E3
Guest contributions: Cardano Foundation × GLEIF; Blindsight
Foreword: Tim Grant, Executive Chairman, TensorX
Launch: {CONFIG["VENUE"]}, {CONFIG["DATE"]}
...
if PRELAUNCH:
    ...
    llms += (f"- [Full report (PDF)]({SITE}/{CONFIG['PDF']}): 55 pages, including sources and the co-authors' disclaimer\n"
             f"- [Full report (Markdown)]({SITE}/agentic-finance-report.md): every chapter, guest contribution, source and the disclaimer, with printed page markers\n"
...
- [Cardano Foundation](https://cardanofoundation.org) × [GLEIF](https://www.gleif.org) (guest): verifiable organisational identity for autonomous agents
- [Blindsight](https://blindsight.io) (guest): runtime protection for AI agents against prompt injection and data poisoning
